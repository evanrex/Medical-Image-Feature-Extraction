...
#########################
parser.add_argument('--evaluate_on_test', dest='evaluate_on_test', action='store_true', help='evaluate model on test set')
parser.add_argument('--pretrained_linear_weights', default='', type=str, help="Path to pretrained linear weights to evaluate.")
...
def main():
    ...
    val_dataset = datasets.ImageFolder(os.path.join(args.data_path, "validation"))
    ...
    if args.evaluate_on_test:
        ...
        dataset_test = datasets.ImageFolder(os.path.join(args.data_path, "test"))
        ...
        losses_avg, top1_avg = test_network(test_loader, model, linear_classifier)
        
        print("==== Losses average: {} | top1 average: {} ====".format(losses_avg, top1_avg))
        
        
        print("========== Done Evaluating Model on Test partition ==========")
        return
...
def test_network(test_loader, model, linear_classifier):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    # top5 = AverageMeter()
    ...
    with torch.no_grad():
        ...
        for i, (inp, target) in enumerate(test_loader):
            ...
            top1.update(acc1[0], inp.size(0))
            # top5.update(acc1[0], inp.size(0))
            print('* Acc@1 {} | loss {}'.format(acc1[0], loss.item()))

            # measure elapsed time
            batch_time.update(time.perf_counter() - end)
            end = time.perf_counter()
            
            probabilities = nn.functional.softmax(output, dim=1)[:, 1]
            y_score = probabilities.detach().cpu().numpy()#force=True)
            y_scores = np.append(y_scores,y_score)
            
            y_labels = target.detach().cpu().numpy()#force=True)
    ...
    np.savetxt(os.path.join(args.dump_path, "true_labels.csv"), y_true, delimiter=",")
    np.savetxt(os.path.join(args.dump_path, "predicted_labels.csv"), y_scores, delimiter=",")
    
    return losses.avg, top1.avg.item() #, top5.avg.item()

def load_pretrained_linear_weights(linear_classifier, pretrained_linear_weights):
    checkpoint = torch.load(pretrained_linear_weights)
